...
<head>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .thumb { width:72px; height:52px; border-radius:.4rem; object-fit:cover; border:1px solid #e4e9f0; }
  </style>
</head>
<body>
  ...
  <div class="main" id="mainContent">
    ...
    <div class="content">
      ...
      <div class="section-card">
        <div class="section-head">
          <h6><i class="bi bi-door-open me-1" style="color:var(--primary)"></i> Kelola Kamar</h6>
          <a href="{{ route('owner.kamar.create') }}" class="btn-tambah">
              ...
              <tr>
                ...
                <td>
                  @if($room->mainImage?->foto_path)
                    <img src="{{ '/storage/'.ltrim($room->mainImage->foto_path, '/') }}" class="thumb" alt="Foto kamar">
                  @else
                    <span class="text-muted">-</span>
                  @endif
                </td>
                <td class="fw-semibold">{{ $room->kost->nama_kost }}</td>
                <td>{{ $room->nomor_kamar }}</td>
                <td>
    @if($room->aktif_bulanan && $room->harga_per_bulan)
        <div>Rp {{ number_format($room->harga_per_bulan, 0, ',', '.') }} <small class="text-muted">/ bulan</small></div>
    @endif
    @if($room->aktif_harian && $room->harga_harian)
        <div>Rp {{ number_format($room->harga_harian, 0, ',', '.') }} <small class="text-muted">/ hari</small></div>
    @endif
    ...
    @endif
                ...
                <td>
                  <a href="{{ route('owner.kamar.show', $room->id_room) }}" class="btn btn-sm btn-info text-white">
                    <i class="bi bi-eye"></i>
                  </a>
                  <a href="{{ route('owner.kamar.edit', $room->id_room) }}" class="btn btn-sm btn-warning">
                    <i class="bi bi-pencil"></i> Edit
                  </a>
                  <form action="{{ route('owner.kamar.destroy', $room->id_room) }}" method="POST" class="d-inline js-confirm-delete" data-delete-message="Anda yakin ingin menghapus kamar ini?">
                    @csrf
                    @method('DELETE')
    ...
    <footer class="owner-footer">
      &copy; {{ date('Y') }} KostFinder - Panel Pemilik Kost.
    </footer>
  </div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    newInput.addEventListener('input', function () {
      ...
      rows.forEach(row => {
        const nomorKamar = row.querySelector('td:nth-child(2)')?.textContent.toLowerCase() || '';
        const namaKost   = row.querySelector('td:nth-child(3)')?.textContent.toLowerCase() || '';
        const tipe       = row.querySelector('td:nth-child(4)')?.textContent.toLowerCase() || '';

        row.style.display = (
          !keyword ||
          nomorKamar.includes(keyword) ||
