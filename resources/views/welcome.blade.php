...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .hero {
    ...
    overflow-x: hidden;
    overflow-y: visible; /* agar dropdown tidak terpotong */
  }
  .hero-bg {
  ...
  .hero-mesh {
    position: absolute; inset: 0;
    background:
      radial-gradient(ellipse 60% 55% at 80% 30%, rgba(228,87,46,.22) 0%, transparent 65%),
      radial-gradient(ellipse 40% 60% at 10% 80%, rgba(255,179,71,.12) 0%, transparent 55%);
  ...
  .hero-content {
    ...
    padding-top: 2.5rem; /* jarak dari navbar tidak terlalu besar */
    padding-bottom: 5rem;
  ...
  .hero-badge {
    ...
    backdrop-filter: blur(8px); color: #e8d5c8; font-size: .8rem; font-weight: 600;
    letter-spacing: .03em; margin-bottom: 1.4rem;
    animation: heroIn .9s .1s cubic-bezier(.22,1,.36,1) both;
  }
  .hero-badge span.dot {
  ...
  .hero-title {
    ...
    font-size: clamp(2.4rem, 5.5vw, 4.8rem); line-height: 1.06;
    letter-spacing: -.025em; color: #fff; margin-bottom: 1rem;
    animation: heroIn .9s .18s cubic-bezier(.22,1,.36,1) both;
  }
  .hero-title .accent { color: var(--kf-primary-light); font-style: italic; position: relative; }
  .hero-title .accent::after {
    content:''; position:absolute; bottom:-4px; left:0; right:0;
    height:3px; background:linear-gradient(90deg,var(--kf-primary-light),transparent); border-radius:99px;
  }
  .hero-sub {
    color: #bdc9e0; font-size: 1rem; max-width: 580px; line-height: 1.75;
    margin-bottom: 2rem;
    animation: heroIn .9s .26s cubic-bezier(.22,1,.36,1) both;
  }

  /* ══ SEARCH ══ */
  .search-wrapper {
    position: relative; max-width: 760px;
    z-index: 9999; /* tinggi agar dropdown overlay konten bawah */
    animation: heroIn .9s .34s cubic-bezier(.22,1,.36,1) both;
  }
  .search-card {
    background: rgba(255,255,255,.99); backdrop-filter: blur(16px);
    border-radius: 1.5rem; padding: .95rem 1rem;
    box-shadow: 0 24px 48px rgba(5,10,20,.22), 0 0 0 1px rgba(255,255,255,.2);
  }
  .search-card .form-control {
    border:0; height:50px; font-size:.95rem; padding-left:0;
    color:var(--kf-dark); background:transparent;
  }
  ...
  .btn-filter {
    ...
    padding:.58rem .95rem; border-radius:.95rem;
    border:1px solid #f1d7cb;
    background:linear-gradient(135deg,#fff8f4 0%,#ffece3 100%);
    color:#f06432; font-size:.84rem; font-weight:700; white-space:nowrap;
    cursor:pointer; transition:all .18s;
  }
  .btn-filter:hover { background:linear-gradient(135deg,#fff2eb 0%,#ffe1d4 100%); border-color:#efb7a2; }
  .btn-cari {
    height:50px; padding:0 1.6rem; border-radius:1rem; border:0;
    background:linear-gradient(135deg,#ffd7c7 0%,#ff9b75 100%);
    color:#31170d; font-weight:800; font-size:.95rem; letter-spacing:.01em;
    white-space:nowrap; box-shadow:0 10px 24px rgba(228,87,46,.22);
  ...
  .hero-chip {
    ...
    color:#c8d8f0; cursor:pointer; font-size:.76rem; font-weight:600;
    transition:all .18s ease;
  }
  .hero-chip:hover { border-color:rgba(228,87,46,.5); color:#fff; background:rgba(228,87,46,.22); }
  .hero-kota-thumb {
    width:88px; height:56px; border-radius:.75rem; overflow:hidden;
    background:rgba(255,255,255,.08); margin-bottom:.38rem;
  ...
  .quick-tags {
    ...
    animation:heroIn .9s .42s cubic-bezier(.22,1,.36,1) both;
  }
  .qtag {
    display:inline-flex; align-items:center; gap:.3rem;
    padding:.3rem .75rem; border-radius:999px;
    background:rgba(255,255,255,.12); border:1px solid rgba(255,255,255,.2);
    color:#e0e8f5; font-size:.78rem; font-weight:600;
    cursor:pointer; backdrop-filter:blur(6px); transition:all .2s;
  }
  .qtag:hover { background:rgba(255,255,255,.2); color:#fff; }

  /* ══ STATS — blur hitam putih transparan campur ══ */
  .stats-row { position:relative; z-index:4; margin-top:-3rem; }
  .stats-card {
    /* efek blur hitam-putih transparan sesuai keinginan */
    background: rgba(255,255,255,0.10);
    backdrop-filter: blur(22px) grayscale(0.35) brightness(1.15);
    -webkit-backdrop-filter: blur(22px) grayscale(0.35) brightness(1.15);
  ...
  .stat-number .stat-accent { color:var(--kf-primary-light); }
  .stat-label { margin-top:.3rem; color:rgba(255,255,255,.7); font-size:.82rem; font-weight:500; }
  ...
  .kota-section::before { content:''; position:absolute; top:-100px; left:50%; transform:translateX(-50%); width:600px; height:300px; background:radial-gradient(ellipse,rgba(228,87,46,.025) 0%,transparent 70%); pointer-events:none; }
  ...
  /* ══ REKOMENDASI — card persegi panjang ke bawah ══ */
  .reco-section { background:#f7f4f1; position:relative; overflow:hidden; }
  .reco-section::before {
    content:''; position:absolute; top:-80px; right:-80px;
    width:300px; height:300px; border-radius:50%;
    background:radial-gradient(circle,rgba(228,87,46,.07) 0%,transparent 70%);
    pointer-events:none;
  }

  .reco-track {
    display:flex; gap:1rem; overflow-x:auto;
    scroll-behavior:smooth; padding-bottom:.75rem; scrollbar-width:none;
    /* agar card tingginya sama */
    align-items: stretch;
  }
  .reco-track::-webkit-scrollbar { display:none; }

  /* FIX: lebar card dikecilkan agar lebih persegi, tinggi mengikuti konten */
  .reco-item { flex:0 0 185px; display:flex; }
  .reco-item > a { width:100%; text-decoration:none; color:inherit; display:flex; }

  .kost-card {
    width:100%; border-radius:1.1rem; overflow:hidden;
    background:#fff; border:1px solid #ede8e5;
    box-shadow:0 4px 18px rgba(228,87,46,.06);
    transition:transform .25s,box-shadow .25s;
    display:flex; flex-direction:column;
  }
  .kost-card:hover { transform:translateY(-6px); box-shadow:0 18px 40px rgba(228,87,46,.14); }

  /* FIX: gambar dikecilkan tingginya agar lebih banyak teks kelihatan */
  .kost-img-wrap { position:relative; height:140px; overflow:hidden; flex-shrink:0; }
  .kost-img-wrap img { width:100%; height:100%; object-fit:cover; object-position:center; transition:transform .45s; }
  .kost-card:hover .kost-img-wrap img { transform:scale(1.06); }
  ...
  .btn-fav i { font-size:.85rem; transition:transform .25s cubic-bezier(.34,1.56,.64,1),color .15s; }
  .btn-fav.pop i { transform:scale(1.5); }

  /* FIX: body card dengan padding compact dan teks tetap kelihatan */
  .kost-body { padding:.8rem .85rem .9rem; flex:1; display:flex; flex-direction:column; gap:.18rem; }

  .kost-badge-row { display:flex; align-items:center; gap:.3rem; flex-wrap:wrap; margin-bottom:.2rem; }
  .kost-badge-tipe {
    background:rgba(228,87,46,.1); color:var(--kf-primary);
    font-size:.63rem; font-weight:800; padding:.1rem .4rem;
    border-radius:.35rem; white-space:nowrap;
  }
  .kost-badge-rating { font-size:.63rem; font-weight:700; color:#f59e0b; }
  .kost-badge-tersedia { font-size:.63rem; font-weight:700; color:#16a34a; margin-left:auto; }

  .kost-name { font-weight:800; font-size:.84rem; color:var(--kf-dark); line-height:1.3; }

  /* FIX: alamat tampil 2 baris */
  .kost-loc {
    font-size:.7rem; color:var(--kf-muted);
    display:flex; align-items:flex-start; gap:.18rem; line-height:1.4;
    overflow:hidden; display:-webkit-box;
    -webkit-line-clamp:2; -webkit-box-orient:vertical;
  }
  .kost-loc-icon { font-size:.65rem; color:var(--kf-primary); flex-shrink:0; margin-top:.1rem; }

  /* FIX: fasilitas tampil 2 baris */
  .kost-fas {
    font-size:.67rem; color:#9ca3af; line-height:1.5;
    overflow:hidden; display:-webkit-box;
    -webkit-line-clamp:2; -webkit-box-orient:vertical;
  ...
  /* Harga di bawah */
  .kost-price-wrap { margin-top:auto; padding-top:.55rem; border-top:1px solid #f5ede8; }
  .kost-price-bulanan { font-size:.95rem; font-weight:900; color:var(--kf-primary); }
  .kost-price-harian { font-size:.85rem; font-weight:800; color:#f59e0b; }
  .kost-price-unit { font-size:.68rem; color:#9ca3af; }
  ...
  /* ══ OWNER BANNER — warna kaya oranye gelap ══ */
  ...
  .owner-banner {
    ...
    box-shadow:0 24px 64px rgba(228,87,46,.3);
  }
  /* dekorasi lingkaran cahaya */
  .owner-banner::before {
    content:''; position:absolute; top:-100px; right:-100px;
    width:450px; height:450px; border-radius:50%;
    background:radial-gradient(circle,rgba(255,120,50,.3) 0%,transparent 65%);
    pointer-events:none;
  }
  .owner-banner::after {
    content:''; position:absolute; bottom:-80px; left:25%;
    width:300px; height:300px; border-radius:50%;
    background:radial-gradient(circle,rgba(255,180,70,.12) 0%,transparent 65%);
    pointer-events:none;
  }
  .owner-banner-has-img {
    background-image:
      linear-gradient(120deg, rgba(20,5,0,.97) 0%, rgba(55,15,0,.93) 38%, rgba(110,35,0,.80) 62%, rgba(200,65,20,.5) 85%, transparent 100%),
  ...
  .owner-chip {
    ...
    color:#ffd4bf; font-size:.75rem; font-weight:700; letter-spacing:.07em;
    text-transform:uppercase; padding:.32rem .85rem; border-radius:999px;
    margin-bottom:1.1rem; backdrop-filter:blur(8px);
  }
  .owner-title {
    font-family:'Fraunces',serif; font-weight:700; color:#ffffff;
    font-size:clamp(1.7rem,3.5vw,2.8rem); line-height:1.12;
  ...
  .btn-owner {
    ...
    background:#ffffff; color:var(--kf-primary-dark);
    font-weight:800; font-size:.95rem;
    padding:.9rem 1.8rem; border-radius:.9rem; text-decoration:none;
    box-shadow:0 8px 28px rgba(0,0,0,.28); transition:all .2s;
  }
  .btn-owner:hover { transform:translateY(-2px); color:var(--kf-primary); background:#fff8f4; box-shadow:0 14px 36px rgba(0,0,0,.32); }

  /* ══ FITUR ══ */
  .fitur-section { background:#fff; position:relative; overflow:hidden; }
  .fitur-section::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 55% 45% at 5% 10%,rgba(228,87,46,.03) 0%,transparent 60%),radial-gradient(ellipse 40% 50% at 95% 90%,rgba(255,179,71,.025) 0%,transparent 55%); pointer-events:none; }
  .fitur-inner { position:relative; z-index:2; }
  .fitur-desc-box { padding-right:2.5rem; }
  .fitur-platform-badge { display:inline-flex; align-items:center; gap:.4rem; padding:.32rem .85rem; border-radius:999px; background:rgba(228,87,46,.04); border:1px solid rgba(228,87,46,.15); color:#c03e1c; font-size:.75rem; font-weight:700; letter-spacing:.06em; text-transform:uppercase; margin-bottom:1.1rem; }
  ...
  .fitur-toggle-btn { display:inline-flex; align-items:center; gap:.6rem; padding:.72rem 1.4rem; border-radius:.85rem; border:1.5px solid #ede8e5; background:#fff; color:#374151; font-size:.88rem; font-weight:700; cursor:pointer; transition:all .22s; white-space:nowrap; box-shadow:0 2px 8px rgba(228,87,46,.04); }
  .fitur-toggle-btn:hover { border-color:rgba(228,87,46,.4); color:var(--kf-primary); }
  ...
  .fitur-card { background:#fff; border:1px solid #edeae8; border-radius:1rem; padding:1.1rem 1.2rem; display:flex; gap:.9rem; align-items:flex-start; transition:border-color .22s,transform .22s; }
  .fitur-card:hover { border-color:rgba(228,87,46,.3); transform:translateY(-2px); box-shadow:0 8px 24px rgba(228,87,46,.04); }
  ...
  .fci3{background:linear-gradient(135deg,#ff9b52,#e47a2e);}
  .fci4{background:linear-gradient(135deg,#fb923c,#ea580c);}
  ...
  .final-cta-box { position:relative; overflow:hidden; border-radius:1.5rem; padding:3.2rem 2rem; background:#fff; border:1px solid #edeae8; text-align:center; }
  .final-cta-box::before { content:''; position:absolute; inset:0; background:radial-gradient(circle at top right,rgba(228,87,46,.025),transparent 35%),radial-gradient(circle at bottom left,rgba(255,179,71,.04),transparent 35%); pointer-events:none; }
  .final-cta-title { font-family:'Fraunces',serif; font-size:clamp(1.8rem,3.8vw,3rem); color:#0f1923; font-weight:700; letter-spacing:-.02em; margin-bottom:.8rem; position:relative; }
  .final-cta-sub { max-width:650px; margin:0 auto 1.6rem; color:#6c768a; font-size:.95rem; line-height:1.8; position:relative; }
  .btn-final-cta { display:inline-flex; align-items:center; gap:.5rem; background:linear-gradient(135deg,var(--kf-primary),var(--kf-primary-dark)); color:#fff; font-weight:800; font-size:.95rem; padding:.9rem 1.5rem; border-radius:.85rem; text-decoration:none; box-shadow:0 10px 28px rgba(228,87,46,.3); transition:all .2s ease; position:relative; }
  .btn-final-cta:hover { transform:translateY(-2px); color:#fff; box-shadow:0 14px 36px rgba(228,87,46,.4); }
  ...
  .modal-filter-body::-webkit-scrollbar-track { background:#fdf8f5; }
  ...
  @media(max-width:767px){
    ...
    .kost-img-wrap { height:120px; }
    .kost-body { padding:.65rem .7rem .75rem; }
    .kost-name { font-size:.8rem; }
    .owner-banner { padding:1.8rem 1.1rem; }
    .owner-title { font-size:1.35rem; }
    .final-cta-box { padding:2rem 1rem; }
...
{{-- ══ STATS — blur hitam putih transparan ══ --}}
...
{{-- ══ REKOMENDASI — card persegi panjang ke bawah ══ --}}
<section class="section-space reco-section">
  <div class="container">
    ...
    <div class="reco-track" id="recoTrack">
      ...
      @php
        ...
        $fasilitas = collect($room->fasilitas ?? [])->take(6);
        ...
        <a href="{{ route('kost.show', $kost->id_kost) }}">
          <div class="kost-card">
            ...
            <div class="kost-img-wrap">
              @if($fotoKamar)
                <img src="{{ asset('storage/'.$fotoKamar) }}" alt="{{ $room->nomor_kamar }}" loading="lazy">
              @elseif($kost->foto_utama)
                <img src="{{ asset('storage/'.$kost->foto_utama) }}" alt="{{ $kost->nama_kost }}" loading="lazy">
              @else
                <div style="width:100%;height:100%;background:linear-gradient(135deg,#fff0eb,#ffddd0);display:flex;align-items:center;justify-content:center;font-size:2.5rem;">🛏️</div>
              @endif
            ...
            {{-- Body --}}
            <div class="kost-body">
              <div class="kost-badge-row">
                <span class="kost-badge-tipe">{{ $kost->tipe_kost ?? 'Campur' }}</span>
                @if($rating > 0)
                <span class="kost-badge-rating">⭐ {{ number_format($rating,1) }}</span>
                @endif
                <span class="kost-badge-tersedia">● Tersedia</span>
              </div>

              <div class="kost-name">{{ $kost->nama_kost }}</div>

              <div class="kost-loc">
                <i class="bi bi-geo-alt kost-loc-icon"></i>
                <span>{{ $kost->alamat ?? $kost->kota }}</span>
              </div>
              ...
              <div class="kost-price-wrap">
                @if($room->aktif_bulanan && $room->harga_per_bulan > 0)
                <div>
                  <span class="kost-price-bulanan">Rp {{ number_format($room->harga_per_bulan,0,',','.') }}</span>
                  <span class="kost-price-unit">/bln</span>
                </div>
                @endif
      ...
      @empty
      <div class="p-4 text-muted" style="font-size:.88rem;">Belum ada kamar tersedia saat ini.</div>
      @endforelse
    </div>
  </div>
</section>

{{-- ══ OWNER BANNER — warna oranye gelap kaya ══ --}}
  ...
  <div class="container fitur-inner">
    <div class="fitur-layout reveal">
      ...
      <div>
        ...
        <div id="fiturPlaceholder" style="padding:1.4rem 0;color:#8a8a8a;font-size:.88rem;line-height:1.8;">
          Klik tombol <strong style="color:var(--kf-primary)">Lihat Semua Fitur</strong> untuk melihat apa saja yang bisa kamu manfaatkan di KostFinder. 🚀
        </div>
      </div>
    </div>
  </div>
...
<section class="final-cta-section section-space">
  <div class="container reveal">
    <div class="final-cta-box">
      ...
      <a href="{{ route('carikos') }}" class="btn-final-cta">Cari Kost Sekarang <i class="bi bi-arrow-right"></i></a>
    </div>
  </div>
</section>
        ...
        <div class="filter-group">
          ...
          <div class="d-flex flex-column gap-2">
            ...
            <label style="display:flex;align-items:center;gap:.6rem;font-size:.85rem;cursor:pointer;">
              <input type="radio" name="urutan" value="{{ $val }}" style="accent-color:var(--kf-primary);" {{ $val==='rekomendasi'?'checked':'' }}>
              {{ $label }}
            </label>
            @endforeach
          </div>
...
(function(){
  ...
  if(recoTrack){
    const slide=()=>Math.max(200,recoTrack.clientWidth*.55);
...
function switchHeroTab(tab,el){
  const d=document.getElementById('heroTabDaerah');
  const k=document.getElementById('heroTabKampus');
  ...
  document.querySelectorAll('.hero-tab-bar .btn-link').forEach(b=>{
    b.style.color='rgba(255,255,255,.45)'; b.style.borderBottom='none';
    b.classList.remove('fw-bold'); b.classList.add('fw-medium');
  });
  el.style.color='var(--kf-primary-light)'; el.style.borderBottom='2px solid var(--kf-primary-light)';
...
function resetFilter(){
  ['tipe','harga','durasi','sewa'].forEach(g=>{
    document.querySelectorAll(`.f-chip[data-group="${g}"]`).forEach((c,i)=>c.classList.toggle('aktif',i===0));
  });
  document.querySelectorAll('input[name="lokasi[]"]').forEach(c=>c.checked=false);
  const u=document.querySelector('input[name="urutan"][value="rekomendasi"]');
  if(u) u.checked=true;
  const mn=document.getElementById('filterMinRange'); const mx=document.getElementById('filterMaxRange');
  if(mn) mn.value=0; if(mx) mx.value=15000000;
  updateFilterRange();
}
...
function terapkanFilter(){
  ...
  const minR=document.getElementById('filterMinRange'); const maxR=document.getElementById('filterMaxRange');
...
function toggleSemuaFitur(){
  ...
  const ph=document.getElementById('fiturPlaceholder');
  const isOpen=list.classList.contains('open');
  list.classList.toggle('open',!isOpen);
  btn.classList.toggle('open',!isOpen);
  if(ph) ph.style.display=isOpen?'':'none';
}
