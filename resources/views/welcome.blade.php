...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .hero-content { position: relative; z-index: 2; padding: 1rem 0; animation: heroIn .9s cubic-bezier(.22,1,.36,1) both; }
  ...
  /* ── HERO DROPDOWN ── */
  .hero-dropdown { position: absolute; top: calc(100% + 8px); left: 0; right: 0; z-index: 99999; background: #fff; border-radius: 1.35rem; border: 1px solid #eef2f7; box-shadow: 0 22px 54px rgba(10,20,50,.18); padding: 1rem 1rem .95rem; animation: dropIn .2s ease; }
  ...
  .stats-card { background: #fff; border-radius: 1.2rem; border: 1px solid #ede8e5; box-shadow: 0 20px 50px rgba(228,87,46,.04); padding: 1.5rem 2rem; }
  ...
  .stat-divider { width: 1px; background: #ede8e5; align-self: stretch; }
  ...
  /* ── KOTA POPULER — putih + efek orange ── */
  ...
  .reco-item { flex: 0 0 260px; display: flex; }
  .reco-item > a { width: 100%; }
  .kost-card { border-radius: 1rem; overflow: hidden; background: #fff; border: 1px solid #ede8e5; box-shadow: 0 4px 16px rgba(228,87,46,.03); transition: transform .25s, box-shadow .25s; }
  .kost-card:hover { transform: translateY(-5px); box-shadow: 0 16px 40px rgba(228,87,46,.12); }
  .kost-img-wrap { position: relative; height: 170px; overflow: hidden; }
  ...
  .owner-banner::before {
    ...
    background:
      radial-gradient(ellipse 55% 80% at 90% 50%, rgba(228,87,46,.04), transparent),
      radial-gradient(ellipse 40% 60% at 5% 20%, rgba(255,179,71,.03), transparent);
    pointer-events: none;
  }
  /* foto background di banner pakai gambar jika ada */
  .owner-banner-has-img {
    background-image:
      linear-gradient(100deg, rgba(255,245,240,.98) 0%, rgba(255,245,240,.92) 40%, rgba(255,245,240,.6) 70%, rgba(255,245,240,.15) 100%),
      url('{{ asset("images/banner/owner-kost-banner.jpg") }}');
    background-size: cover; background-position: center right;
  }
  .owner-content { position: relative; z-index: 2; max-width: 580px; }
  .owner-chip { display: inline-flex; align-items: center; gap: .4rem; background: rgba(228,87,46,.04); border: 1px solid rgba(228,87,46,.15); color: #c03e1c; font-size: .75rem; font-weight: 700; letter-spacing: .07em; text-transform: uppercase; padding: .3rem .8rem; border-radius: 999px; margin-bottom: 1rem; }
  .owner-title { font-family: 'Fraunces', serif; font-weight: 700; color: #0f1923; font-size: clamp(1.7rem, 3.5vw, 2.9rem); line-height: 1.12; letter-spacing: -.02em; margin-bottom: .7rem; }
  .owner-sub { color: #6c768a; font-size: .95rem; line-height: 1.6; margin-bottom: 1.6rem; }
  .btn-owner { display: inline-flex; align-items: center; gap: .5rem; background: linear-gradient(135deg, var(--kf-primary), var(--kf-primary-dark)); color: #fff; font-weight: 800; font-size: .95rem; padding: .85rem 1.6rem; border-radius: .85rem; text-decoration: none; box-shadow: 0 8px 24px rgba(228,87,46,.3); transition: all .2s; }
  .btn-owner:hover { transform: translateY(-2px); color: #fff; box-shadow: 0 12px 32px rgba(228,87,46,.4); }

  /* ── FITUR SECTION — putih + aksen orange tipis ── */
  .fitur-section {
    background: #fff;
  ...
  .fitur-section::before {
    content: ''; position: absolute; inset: 0;
    background:
      radial-gradient(ellipse 55% 45% at 5% 10%, rgba(228,87,46,.03) 0%, transparent 60%),
      radial-gradient(ellipse 40% 50% at 95% 90%, rgba(255,179,71,.025) 0%, transparent 55%);
    pointer-events: none;
  }
  ...
  .fitur-toggle-btn:hover { background: #fff; border-color: rgba(228,87,46,.4); color: var(--kf-primary); }
  ...
  .fitur-card { background: #fff; border: 1px solid #edeae8; border-radius: 1rem; padding: 1.1rem 1.2rem; display: flex; gap: .9rem; align-items: flex-start; transition: background .22s, border-color .22s, transform .22s; }
  .fitur-card:hover { background: #fff; border-color: rgba(228,87,46,.3); transform: translateY(-2px); box-shadow: 0 8px 24px rgba(228,87,46,.04); }
  ...
  /* ── TESTIMONIAL — hapus biru, pakai putih/abu ── */
  .testimonial-section { background: #fff; }
  .testimonial-card { background: #fff; border: 1px solid #ede8e5; border-radius: 1.2rem; padding: 1.5rem; height: 100%; box-shadow: 0 6px 24px rgba(228,87,46,.025); transition: all .25s ease; }
  .testimonial-card:hover { transform: translateY(-5px); box-shadow: 0 16px 36px rgba(228,87,46,.025); }
  ...
  /* ── FINAL CTA — oranye keputihan ── */
  .final-cta-section { background: #fff; }
  .final-cta-box {
    position: relative; overflow: hidden; border-radius: 1.5rem; padding: 3.2rem 2rem;
    background: #fff;
    border: 1px solid #edeae8;
    text-align: center;
  }
  .final-cta-box::before {
    content: ''; position: absolute; inset: 0;
    background:
      radial-gradient(circle at top right, rgba(228,87,46,.025), transparent 35%),
      radial-gradient(circle at bottom left, rgba(255,179,71,.04), transparent 35%);
    pointer-events: none;
  }
...
<div class="welcome-page">
  ...
  {{-- ══ REKOMENDASI — background abu muda bukan biru ══ --}}
          ...
          <a href="{{ route('kost.show', $kost->id_kost) }}" style="text-decoration:none;color:inherit;display:block;width:100%;">
            <div class="kost-card" style="height:100%;display:flex;flex-direction:column;">
              <div class="kost-img-wrap" style="flex-shrink:0;">
                ...
                @else
                  <div style="width:100%;height:100%;background:linear-gradient(135deg,#fff0eb,#ffddd0);display:flex;align-items:center;justify-content:center;font-size:3.5rem;">🛏️</div>
                @endif
              ...
              <div class="kost-body" style="flex:1;display:flex;flex-direction:column;">
                <div style="display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;margin-bottom:.35rem;">
                  <span style="background:rgba(15,25,40,.08);color:#374151;font-size:.7rem;font-weight:700;padding:.15rem .5rem;border-radius:.4rem;">
                    {{ $kost->tipe_kost ?? 'Campur' }}
                  </span>
                  @php $rating = $room->kost->reviews_avg_rating ?? 0; @endphp
                  @if($rating > 0)
                  <span style="font-size:.7rem;font-weight:700;color:var(--kf-dark);">⭐ {{ number_format($rating,1) }}</span>
                  @endif
                  <span style="color:#e4572e;font-size:.7rem;font-weight:700;">Tersedia</span>
                </div>
                <div class="kost-name">{{ $kost->nama_kost }}</div>
                <div class="kost-loc">
                  <i class="bi bi-geo-alt" style="font-size:.7rem;color:var(--kf-primary);flex-shrink:0;"></i>
                  <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $kost->alamat ?? $kost->kota }}</span>
                </div>
                @if($fasilitas->count() > 0)
                <div class="kost-fas">{{ $fasilitas->implode(' · ') }}</div>
                @endif
                <div style="margin-top:auto;padding-top:.5rem;display:flex;align-items:baseline;gap:.75rem;flex-wrap:wrap;">
                  @if($room->aktif_bulanan && $room->harga_per_bulan > 0)
                  <div>
                    <span style="font-size:1rem;font-weight:900;color:var(--kf-primary);">Rp {{ number_format($room->harga_per_bulan,0,',','.') }}</span>
                    <span style="font-size:.72rem;color:#9ca3af;">/bln</span>
                  </div>
                  @endif
                  @if($room->aktif_harian && $room->harga_harian > 0)
                    @if($room->aktif_bulanan && $room->harga_per_bulan > 0)
                    <span style="color:#e0e0e0;font-size:.8rem;">|</span>
                    @endif
                    <div>
                      <span style="font-size:.9rem;font-weight:800;color:#f59e0b;">Rp {{ number_format($room->harga_harian,0,',','.') }}</span>
                      <span style="font-size:.72rem;color:#9ca3af;">/hari</span>
                    </div>
                  @endif
                </div>
              </div>
...
function switchHeroTab(tab, el) {
  ...
  if (tab === 'daerah') { daerah.classList.remove('d-none'); daerah.classList.add('d-flex'); kampus.classList.add('d-none'); kampus.classList.remove('d-flex'); }
  else { kampus.classList.remove('d-none'); kampus.classList.add('d-flex'); daerah.classList.add('d-none'); daerah.classList.remove('d-flex'); }
  document.querySelectorAll('#heroDropdown .btn-link').forEach(btn => { btn.style.color='#888'; btn.style.borderBottom='none'; btn.classList.remove('fw-bold'); btn.classList.add('fw-medium'); });
  el.style.color='var(--kf-primary)'; el.style.borderBottom='2px solid var(--kf-primary)'; el.classList.remove('fw-medium'); el.classList.add('fw-bold');
}
document.addEventListener('click', e => {
  const wrapper = document.getElementById('searchWrapper');
  if (wrapper && !wrapper.contains(e.target)) document.getElementById('heroDropdown')?.classList.add('d-none');
});
function doSearch() {
...
function resetFilter() {
  ...
  if (minR) minR.value=0; if (maxR) maxR.value=15000000;
  updateFilterRange();
}
function terapkanFilter() {
  ...
  const minRange=document.getElementById('filterMinRange'); const maxRange=document.getElementById('filterMaxRange');
  if (minRange && parseInt(minRange.value)>0) params.set('harga_min', minRange.value);
  if (maxRange && parseInt(maxRange.value)<15000000) params.set('harga_max', maxRange.value);
